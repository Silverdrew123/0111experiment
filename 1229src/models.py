...
from transformers.models.t5.modeling_t5 import T5ForConditionalGeneration,T5EncoderModel
...
class LMForPromptFinetuning(BertPreTrainedModel):
    ...
    def freeze_lora_component(self):
    ...
    def get_proto_embedding(self,
                    ...
                    block_flag=None,
                    example_id=None):
        _,embedding=self.lm_model.encode(input_ids, attention_mask, mask_pos, inputs_embeds)
        embedding=self.lm_model.proto_head(embedding)
        return embedding
    ...
    def get_constrast_loss(self,
        ...
        _, sequence_mask_output_1 = self.lm_model.encode(input_ids, attention_mask, mask_pos, inputs_embeds)
        _, sequence_mask_output_2 = self.lm_model.encode(input_ids, attention_mask, mask_pos, inputs_embeds)

        sequence_mask_output_1= self.lm_model.lm_head.dense(sequence_mask_output_1)
        sequence_mask_output_2 = self.lm_model.lm_head.dense(sequence_mask_output_2)
        ...
        contra_loss = self.lm_model.contra_lc(sequence_mask_output_1.unsqueeze(1), sequence_mask_output_1.unsqueeze(0), pair_labels)
        #contra_loss = self.lm_model.contra_lc(sequence_mask_output_1.unsqueeze(1), sequence_mask_output_2.unsqueeze(0), pair_labels)
    ...
    def forward(
        ...
        return ((loss,) + output+ (sequence_mask_output,)) if loss is not None else (output+(sequence_mask_output,))#wxy
...
class T5ForPromptFinetuning(T5EncoderModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.lm_head = T5LMHead(config)
        if 1:
            self.proto_head = nn.Linear(config.hidden_size, 768)
        else:
    ...
    def encode(self, input_ids=None, attention_mask=None, mask_pos=None, inputs_embeds=None,token_type_ids=None, 
