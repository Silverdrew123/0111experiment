...
class newOurInputFeatures(InputFeatures):
    """
...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        if self.features is None:
            ...
            supports2 = self.select_context([self.support_examples[i] for i in context_indices])
    ...
    def new_convert_fn(
        ...
        if not use_demo:
            ...
            template2_list = self.args.template_list_new
            template2 = random.choice(template2_list)
            ...
            #####  the 1st view of input for contrastive learning.
            #####  the 1st view of input is by replacing with a different template2
            

            ###### augmented_example for view1
            ...
            # in-context learning, the input (query) might be at the end instead of the beginning (gpt3_in_context_head)
            augmented_example1 = []
            # template1_list = self.args.template_list_new
            # template1 = random.choice(template1_list)
            # query_text1 = input_example_to_tuple(example) # Input sentence list for query
            # support_by_label1 = [[] for i in range(len(label_map))]
            ...
            #             pass
            #         else:
            #             for support_example1 in filter(lambda s: s.label == label_name, supports):   ### we have supports and supports2
            #                 support_by_label1[label_id] += input_example_to_tuple(support_example1)

            #     augmented_example1 = query_text1
            #     for label_id in range(len(label_map)):
            #         augmented_example1 += support_by_label1[label_id]

            # view1_inputs = tokenize_multipart_input(
            #     input_text_list=augmented_example1,
            #     max_length=max_length,
            #     tokenizer=self.tokenizer,
            #     task_name=self.args.task_name,
            #     prompt=prompt,
            #     template=template1,     ### use the sampled template
            #     label_word_list=label_word_list,
            #     first_sent_limit=self.args.first_sent_limit,
            #     other_sent_limit=self.args.other_sent_limit,
            #     truncate_head=self.args.truncate_head,
            ...
            #####  the 2nd view of input for contrastive learning.
            #####  the 2nd view of input is by replacing with a different template2
            template2 = self.args.template_list_new
            # template2 = random.choice(template2_list)
            ...
            features = newOurInputFeatures(inputs['input_ids'], inputs['attention_mask'], inputs['token_type_ids'], example_label, inputs['mask_pos'], None,
                                        inputs['input_ids'], inputs['attention_mask'], inputs['token_type_ids'], example_label, inputs['mask_pos'], None,
                                        view2_inputs['input_ids'], view2_inputs['attention_mask'], view2_inputs['token_type_ids'], example_label, view2_inputs['mask_pos'], None)
