...
# from package.arg import HfArgumentParser
...
MAPPING = {'mnli': "{'contradiction':'No','entailment':'Yes','neutral':'Maybe'}",
           ...
           "sst-5": "{0: 'disgust', 1: 'simple', 2: 'hilarious', 3: 'fascinating', 4:'magnificent'}",
           "ag_news": "{1: 'politics', 2: 'sports', 3: 'finance', 4:'technology'}",
           }
TEMPLATE={"mnli":"*cls**sent-_0*?*mask*,*+sentl_1**sep+*",
          'sst-2':"*cls**sent_0*_It_was*mask*.*sep+*",
           ...
           'sst-5':"*cls**sent_0*_A*mask*_movie.*sep+*",
           'ag_news': "*cls**sent_0*_For_more_on*mask*,_click_here.*sep+*",
          }
...
def main():
    ...
    for label_id in range(num_labels):
        ...
        #'*cls**sent_0*_It_was*mask*.*sep+**sent_1*_It_was*label_0*.*sep+**sent_2*_It_was*label_1*.*sep+*'
    ...
    if data_args.template_list_new is not None:
        ...
        old_template = data_args.template_list_new
        new_template = old_template + ''
        old_template = old_template.replace('*cls*', '')
        # Single sentence or sentence pair?
        sent_num = 1
        ...
        for label_id in range(num_labels):
            ...
            sub_template = sub_template.replace("*mask*", "*label_{}*".format(label_id))
            new_template = new_template + sub_template
        logger.info("| {} => {}".format(data_args.template_list_new, new_template))

        data_args.template_list_new = new_template
    ...
    def build_compute_metrics_fn(task_name: str) -> Callable[[EvalPrediction], Dict]:
        def compute_metrics_fn(p: EvalPrediction):
            ...
            predictions = p.predictions[0]#wxy
    ...
    checkpoint = os.path.join('self.args.output_dir', 'pytorch_model.bin')
    # checkpoint="self.args.output_dir/checkpoint/449epoch_acc-inf"
    ...
    if data_args.prompt:
        ...
        # model.load_model(checkpoint)wxy
    else:
