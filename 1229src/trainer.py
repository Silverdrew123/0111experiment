...
class Trainer(transformers.Trainer):
    ...
    def pseudo_data_selection(self, model, un_inputs, un_meta,unembedding,learning_rate=None, use_soft_label=None):
        ...
        if proto!=None and self.args.protoweight:
            dis=[]
            if protonet:
    ...
    def get_proto_dataloader(self, percentage=1) -> DataLoader:
        ...
        # self.meta_train_dataset.example_idx = self.meta_dataset.example_idx[:self.meta_train_num]
        # batch_size=self.proto_train_num/self.proto_dataset.num_labels/4
        return DataLoader(
            self.proto_dataset,
            batch_size=self.proto_dataset.num_labels*4,
            sampler=sampler,
            collate_fn=self.data_collator,
            drop_last=self.args.dataloader_drop_last,
            num_workers=self.args.dataloader_num_workers,
    ...
    def training_step(self, model, inputs=None, un_inputs=None) -> torch.Tensor:
        ...
        if inputs is not None:
            ...
            if protonet:
                ...
                if self.protodata_step==1:
                    self.proto_loss=0
                    self.proto_loss+=loss_pro.detach().item()
                else:
                    self.proto_loss+=loss_pro.detach().item()
                    if self.protodata_step==4:
                        self.summaryWriter.add_scalar("protoloss_linear",self.proto_loss/4,self.global_step)
        ...
        if self.args.is_semi == 1 and (un_inputs is not None) and (len(un_inputs) != 0):
            if self.args.use_psuedo_label > 0:
                ...
                soft_labels, hard_labels,un_embedding = self.assign_psuedo_label(un_inputs)#wxy
                un_meta['soft_labels'] = soft_labels
                un_meta['hard_labels'] = hard_labels
                ...
                if mixup:
                    ...
                    for i in range(len(hard_labels)):
                        if max(soft_labels[i])>0.5:
                            ...
                            # dists = euclidean_dist(mixembedding.view(1,mixembedding.shape[0]), self.proto)
                            if 1:
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        self.best_checkpoint=-float("inf")
        if not os.path.exists('self.args.output_dir/checkpoint'):
            # 路径不存在，创建目录
            os.makedirs('self.args.output_dir/checkpoint')
        start_time = time.time()
        self.state.max_steps = self.args.max_steps
        self.dev_objective = dev_objective if dev_objective is not None else default_dev_objective
        self.teacherobjective=-float("inf")
        # Data loading.
        ...
        lora=1
        if lora:
            self.model.freeze_lora_component()
        ...
        if not args.ignore_data_skip:
            for epoch in range(epochs_trained):
                ...
                for _ in train_dataloader:
                    break
        if 0:
            protonet_iterator = trange(
                0, 20, desc="Epoch"
            )
            for epoch in protonet_iterator:
                for step,proinputs in enumerate(self.pro_dataloader):
                    self.model.train()
                    output=self.model(**proinputs)
                    embedding_all=output[-1]
                    label=proinputs['labels']
                    loss_pro,acc=loss_fn(embedding_all.to(self.accelerator.device), target=label.to(self.accelerator.device),n_support=2,device=self.accelerator.device)
                    self.accelerator.backward(loss_pro,retain_graph=True)
        ...
        session_num = 0
        proto=[]
        self.protodata_step=1#记录protodataloader的step
           
    
                
                
        self.summaryWriter = SummaryWriter("loss/log_proto_agnews")   
        for epoch in train_iterator:
            ...
            # if epoch==70:#wxy
            #     break
            if self.args.use_last_epoch and self.global_step >self_training_start_iter and  self.global_step%200==0 and self.args.protoweight:
            ...
            for step, inputs in enumerate(epoch_iterator):
                if 0:
                    self.summaryWriter.add_graph(self.model, inputs)
                if protonet:
                    self.epochtrained=epoch
                    try:
                        self.proinputs = next(pro_train_iter)
                        self.protodata_step+=1
                    except:
                ...
                if self.args.is_semi == 1 and (delta >= 0):
                    ...
                    if delta == 0:
                        self.model.zero_grad()
                        if self.args.use_psuedo_label > 0:
                            if 1 and self.objective>=self.teacherobjective:
                                self.teacherobjective=self.objective
                ...
                if (finetune and (step + 1) % self.args.gradient_accumulation_steps == 0) or (not finetune and (step + 1) % self.args.un_gradient_accumulation_steps == 0) or (
                    ...
                    if self.global_step % self.args.eval_steps == 0:
                        ...
                        if objective>self.best_checkpoint:
                            self.best_checkpoint=objective
                            torch.save({
                                'epoch': epoch, # 保存迭代次数
                                'model_state_dict': self.model.state_dict(), # 模型的状态
                                'optimizer_state': self.optimizer.state_dict(), # 优化器的状态
                                'acc':self.best_checkpoint,
                            }, 'self.args.output_dir/checkpoint/{}epoch_acc{}'.format(epoch,objective))  # 路径，这里每次保存都会覆盖掉checkpoint文件

                        logger.info("now this model Dev result: {}".format(objective))
                        # if self.global_step % 1000 == 0:
                        #     test=self.evaluate(eval_dataset=self.test_dataset)
                        #     testobj=self.dev_objective(test.metrics)
                        #     logger.info("wxy test result: {}".format(testobj))
                        if objective >= self.objective:
                            logger.info("wxy self.teacher objective result: {}".format(self.teacherobjective))
